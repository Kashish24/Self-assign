...
Used OrderedDict for inferred_schema to ensure the column order is preserved as per the data file.
Extracted ordered_columns from the first row of the sample data to maintain the column order.
Modified the filtering and type inference process to respect the order of columns as they appear in the data file.
These changes ensure that the column order is maintained as per the data file, avoiding any potential issues with the order of columns in the CREATE TABLE query.
'''
...
class VoteDataIngestor:
    ...
    def create_schema_and_table(self, columns):
        ...
        create_table_query += ')'  # Closing the CREATE TABLE query
